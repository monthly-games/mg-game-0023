...
import 'package:game/screens/real_game_screen.dart';
...
class MyApp extends StatelessWidget {
  ...
  static const gameId = 'MG-0023';
  static const gameTitle = 'Colony Frontier';
    ...
    return MultiProvider(
      ...
      child: MaterialApp(
        ...
        routes: {
          ...
          '/game': (_) => const RealGameScreen(), // 실제 게임플레이 화면으로 변경
          ...
          '/colony': (_) => const ColonyScreen(),
          '/battle': (_) => const SessionBattleScreen(), // 슈퍼셀 스타일 세션 배틀
          '/leaderboard': (_) => const SocialCompetitionScreen(), // 소셜 경쟁
        },
...
class _SplashScreenState extends State<SplashScreen> {
  ...
  Widget build(BuildContext context) {
    if (_showIntro) {
      return IntroShowcase(onIntroComplete: _onIntroComplete);
    }
    return AnimatedMainMenu(
      onPlayColony: _navigateToColony,
      onStartGame: _navigateToGame,
      onLevels: _navigateToLevels,
      onTutorial: _navigateToTutorial,
      onRewards: _navigateToRewards,
      onDaily: _navigateToDaily,
      onBattle: _navigateToBattle, // 슈퍼셀 스타일 배틀 추가
    );
  }

  void _navigateToColony() {
    Navigator.of(context).pushReplacementNamed('/colony');
  }
  ...
  void _navigateToRewards() {
    Navigator.of(context).pushReplacementNamed('/retention');
  }

  void _navigateToDaily() {
              ...
              child: Column(
                ...
                children: [
                  ...
                  FilledButton.icon(
                    ...
                    label: const Text('Start Game (Prototype)'),
                  ),
                  const SizedBox(height: 8),
                  FilledButton.icon(
                    key: const ValueKey('play-colony'),
                    onPressed: () => Navigator.of(context).pushNamed('/colony'),
                    icon: const Icon(Icons.rocket_launch_rounded),
                    label: const Text('Play Colony Game'),
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: kDebugMode
                ? const _GameWidgetPlaceholder()
                : GameWidget(game: _FrameLoopGame()),
          ),
        ],
      ),
    );
...
/// Placeholder widget for testing environments where GameWidget initialization fails
class _GameWidgetPlaceholder extends StatelessWidget {
  const _GameWidgetPlaceholder();
      ...
      child: Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              'Engine Loop Placeholder',
              style: Theme.of(context).textTheme.titleLarge,
            ),
            const SizedBox(height: 8),
            const Text('GameWidget is available in release builds'),
          ],
        ),
      ),
    );
